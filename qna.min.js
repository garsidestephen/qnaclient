...
function start(){var callbackFn=function(){if(configurableParams.theme!=chipsOnlyTheme){showQnaLauncher(qnaLauncherID)}
else{showQnaBot(qnaID)}};generateAnswer(configurableParams.botPrimingPhrase,1,callbackFn)}
...
function buildAndInsertBotHTML(callbackFn){$.get("/qna/qna.html",function(html){$htmlBody=$('body');var $html=$(html);$html.each(function(){var $ele=$(this);if($ele.hasClass('js-qna')){$ele.attr('id',qnaID);$ele.attr('data-qna-launcher-id',qnaLauncherID);var $chatWindowCloser=$ele.find('.js-qna-bot__header__close');if($chatWindowCloser.length){$chatWindowCloser.attr('data-qna-launcher-id',qnaLauncherID);$chatWindowCloser.attr('data-qna-chat-window-id',qnaID)}
var $chatWindowStream=$ele.find('.js-qna-bot__stream-wrap__stream');if($chatWindowStream.length){$chatWindowStream.attr('id',qnaStreamID);$chatWindowStream.attr('data-qna-chat-window-id',qnaID)};var $chatWindowBusy=$ele.find('.js-qna-bot__stream-wrap__busy');if($chatWindowBusy.length){$chatWindowBusy.attr('id',qnaBusyID);$chatWindowBusy.attr('data-qna-chat-window-id',qnaID)}};if($ele.hasClass('js-qna-launcher')){$ele.attr('id',qnaLauncherID);$ele.attr('data-qna-chat-window-id',qnaID)}});if(configurableParams.pageContainerId!=null&&configurableParams.pageContainerId!=""){$domLocation=$('#'+configurableParams.pageContainerId)}
...
function parseResponse(data,originalQuestion,callbackFn){hideBusy();var response="",chipsHtml="";if(data!=null&&data.answers!=null&&data.answers.length>0){var topAnswer=data.answers[0];if(topAnswer.score>configurableParams.minimumAcceptableAnswerScore){response=topAnswer.answer;var richMediaRegex=response.match(/\[(.*?)\]\((.*?)\)/g);if(richMediaRegex){response=transformRichMedia(response,richMediaRegex)}
...
function updateStream(content,isFromBot){var html="";if(isFromBot){html=`<div class='qna-bot__stream-wrap__stream__interaction qna-bot__stream-wrap__stream__interaction--from-bot'>${content}</div>`}
...
function setTheme(){var themeName=`qna-theme--${configurableParams.theme}`,positionName=`qna-position--${configurableParams.position}`;$qna.addClass(themeName);if(configurableParams.theme!=chipsOnlyTheme){$qna.addClass(positionName)}
...
return{Init:function(ocpApimSubscriptionKey,urlKey,params){init(ocpApimSubscriptionKey,urlKey,params)}}})();qnaClient.Init(ocpApimSubscriptionKey,urlKey,params)}
