var qnaClient = (function () {
    ...
    function start() {
        var callbackFn = function () {
            if (configurableParams.theme != chipsOnlyTheme) { showQnaLauncher() }
            else { showQnaBot() }
    ...
    function parseResponse(data, originalQuestion, callbackFn) {
        hideBusy(); var response = ""; if (data != null && data.answers != null && data.answers.length > 0) {
            var topAnswer = data.answers[0]; if (topAnswer.score > configurableParams.minimumAcceptableAnswerScore) {
                ...
                var chipsRegex = response.match(/\##chips(.*?)##/g), chipsHtml = ""; if (chipsRegex) { chipsHtml = getChipHtml(response, chipsRegex[0]); response = response.replace(/##chips(.*?)##/g, "") }
            }
