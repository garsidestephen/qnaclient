...
var qnaClient=(function(){var isBrowserSpeechRecognitionEnabled=!1,isMobileDevice=(/Mobi/.test(navigator.userAgent)),screenHeight=$(window).height(),documentHeight=$(document).height(),$htmlBody=null,$stream=null,$streamWrap=null,$qna=null,$qnaBot=null,$input=null,$busy=null,$title=null,$logo=null,$closer=null,$launcher=null,$launcherImage=null,$busyMessage=null,fadeSpeed='fast',currentPageUrlPath=window.location.href.replace(window.location.origin,""),ocp_Apim_Subscription_Key=null,qnaEndpoint=null,previousQuestions=[],previousQuestionsCurrentIndex=0,configurableParams={"title":"","minimumAcceptableAnswerScore":50,"pageContainerId":"","pageContainerClass":"","theme":"default","noAnswerPhrase":"Sorry, I don't understand.","somethingGoneWrongPhrase":"Mmm, something went wrong there, try me again!","loggingEnabled":!0,"displayImagesInline":!1,"botPrimingPhrase":"Hi","logoUrl":"/qna/logo.png","buttonToolTip":"Start a chat","position":"bottom-left","inputPrompt":"What would you like to say?","busyMessage":"Hold on a mo..."};var init=function(ocpApimSubscriptionKey,urlKey,params){if(ocpApimSubscriptionKey==null||ocpApimSubscriptionKey==""||urlKey==null||urlKey==""){log("Missing OCPApimSubscriptionKey or UrlKey")}
...
function initEvents(){$launcher.on('click',function(){hideQnaLauncher();showQnaBot()});$closer.on('click',function(){hideQnaBot();showQnaLauncher()});$input.on('keydown',function(e){checkInputKeyPress(e)})}
...
function checkInputKeyPress(e){if(e.which==13){submitQuestion();return!1}
...
function cyclePreviousQuestions(keyCode){if(previousQuestions.length>0){if(keyCode==38){previousQuestionsCurrentIndex=previousQuestionsCurrentIndex<0?0:previousQuestionsCurrentIndex-=1}
...
function generateAnswer(question,numberOfAnswers,callbackFn){if(question!=null&&question!=""){var top=numberOfAnswers==null||numberOfAnswers==0?1:numberOfAnswers;showBusy();$.ajax({type:"POST",url:qnaEndpoint,beforeSend:function(request){request.setRequestHeader("Ocp-Apim-Subscription-Key",ocp_Apim_Subscription_Key)},contentType:"application/json",data:JSON.stringify({"question":question,"top":top}),success:function(data){parseResponse(data,callbackFn)},error:function(XMLHttpRequest,textStatus,errorThrown){responseError(XMLHttpRequest,textStatus,errorThrown,callbackFn)}})}}
function parseResponse(data,callbackFn){hideBusy();var response="";if(data!=null&&data.answers!=null&&data.answers.length>0){var topAnswer=data.answers[0];if(topAnswer.score>configurableParams.minimumAcceptableAnswerScore){var richMediaRegex=topAnswer.answer.match(/\[(.*?)\]\((.*?)\)/g);response=topAnswer.answer;if(richMediaRegex){response=transformRichMedia(topAnswer.answer,richMediaRegex)}}
else{response=configurableParams.noAnswerPhrase}
...
function transformRichMedia(text,richMedia){for(var i=0;i<richMedia.length;i++){var richMediaElements=richMedia[i].split("]("),richMediaText=richMediaElements[0].replace(/\[|\]/g,""),richMediaLink=richMediaElements[1].replace(/\(|\)/g,""),imageBreak=!configurableParams.displayImagesInline?"<br/>":"";if(richMediaText.length&&richMediaLink.length){if(richMediaLink.match(/\.(gif|jpg|jpeg|tiff|png)$/i)){text=text.replace(`[${richMediaText}]`,"");text=text.replace(`(${richMediaLink})`,`${imageBreak}<img src='${richMediaLink}' alt='${richMediaText}' class='qna-bot__stream-wrap__stream__interaction__speech__media-asset'/>${imageBreak}`)}
...
function updateStream(content,isFromBot){var html="";if(isFromBot){var dateTime=formatDate(new Date());html=`<div class='qna-bot__stream-wrap__stream__interaction qna-bot__stream-wrap__stream__interaction--from-bot'><p class='qna-bot__stream-wrap__stream__interaction__speech'>${content}<span class='qna-bot__stream-wrap__stream__date-time'>${dateTime}</span></p><div class='qna-bot__stream-wrap__stream__interaction__arrow-right'></div></div>`}
