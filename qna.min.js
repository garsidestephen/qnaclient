...
function start(){var callbackFn=function(){if(configurableParams.theme!=chipsOnlyTheme){showQnaLauncher()}
else{showQnaBot()}};generateAnswer(configurableParams.botPrimingPhrase,1,callbackFn)}
...
function initEvents(){$launcher.on('click',function(){hideQnaLauncher();showQnaBot()});$closer.on('click',function(){hideQnaBot();showQnaLauncher();registerUnansweredQuestions();clearUnansweredQuestionsInThisSession()});$input.on('keydown',function(e){checkInputKeyPress(e)});$qna.on('click','.js-qna-chip-click',function(){clickChip($(this))})}
